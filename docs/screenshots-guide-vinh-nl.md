...
- **Ảnh 3 (Cấu hình Token trong Jenkins):** Nếu nhóm cấu hình Global, vào **Manage Jenkins** → **System** → cuộn xuống phần **GitHub** để chụp. NẾU KHÔNG THẤY, nghĩa là nhóm cấu hình trực tiếp trong Job: Hãy vào Job **yas-ci-pipeline** → **Configure** → cuộn đến phần **Branch Sources** (chỗ điền link GitHub repo). Chụp màn hình mục **Credentials** đang chọn đúng ID token của bạn (bên dưới có thể có chữ `Credentials verified`).
