...
\subsection{Cấu hình hiển thị Báo cáo}
Để Jenkins hiển thị được các kết quả sau khi build, nhóm đã cấu hình:

\begin{itemize}[noitemsep]
    \item \textbf{JUnit Plugin}: Đọc các file \texttt{TEST-*.xml} trong thư mục \texttt{target/surefire-reports/} để hiển thị biểu đồ Test Result Trend trực tiếp trên giao diện Jenkins.
    \item \textbf{Archive Artifacts}: Lưu trữ file báo cáo Coverage \texttt{jacoco.xml} sau khi chạy xong để người dùng có thể tải về trực tiếp từ giao diện build của Jenkins.
    \item \textbf{JaCoCo Plugin}: Plugin đọc file \texttt{.exec} trong thư mục target để hiển thị biểu đồ Coverage Trend ngay tại trang chủ của Job.
\end{itemize}
...
\begin{itemize}[noitemsep]
    \item \textbf{SonarQube Scanner Plugin}: Cấu hình Webhook trỏ về \texttt{/sonarqube-webhook/} để kết nối với SonarQube.
\end{itemize}
